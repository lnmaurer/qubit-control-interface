...
#The setup tab
	...
	self.serverListbox.delete(0, Tkinter.END) #if the listbox is already populated, clear it, this is unnescessary at present
        self.serverListbox.insert(0,"<None>")
    ...
    self.serverListbox = Tkinter.Listbox(self.setupTab, height=8, selectmode=Tkinter.MULTIPLE) #todo: is there now ttk version of this? NO
    ...
    makeTracesButton = ttk.Button(self.setupTab, text='make traces for selected', state='disabled', command=makeTraces)
    makeTracesButton.grid(column=2, row=9, sticky='n', padx=5, pady=5)
    
    def norm(self):
      makeTracesButton.config(state='normal')
    self.serverListbox.bind('<<ListboxSelect>>',norm)
...
#The code frame    
  ...
  def newExperiment(self):
    filename = tkFileDialog.asksaveasfilename(filetimes=[('Qubit Experiment File','*.qbexp')], title="New experiment name")
